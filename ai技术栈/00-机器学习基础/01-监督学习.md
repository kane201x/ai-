...
### 案例：多分类 Softmax 回归的手动实现

下方用 NumPy 从零实现 Softmax 回归，对比 sklearn 结果，帮助理解交叉熵梯度。
...
for epoch in range(300):
    logits = X @ W
    P = softmax(logits)
    grad = (1 / n_samples) * X.T @ (P - Y)
    W -= lr * grad

y_pred_np = P.argmax(axis=1)
acc_np = (y_pred_np == y).mean()
print(f"自实现 Softmax 回归准确率: {acc_np:.4f}")

sk = LogisticRegression(C=1e9, max_iter=1000).fit(X, y)
print(f"sklearn 多分类逻辑回归准确率: {sk.score(X, y):.4f}")
```

| 求解方式 | 优点 | 缺点 | 适用场景 |
|---------|------|------|---------|
| 闭式/正规方程 | 一步求解、无超参 | 仅线性、O(d³) 内存 | 小特征数 |
| 梯度下降(自实现) | 透明、可定制 | 需调学习率/迭代 | 教学/研究 |
| sklearn 求解器 | 稳定、支持正则 | 黑盒 | 工程实践 |
...
### 案例：XGBoost / LightGBM / CatBoost 三方对比

三个梯度提升框架的目标函数与工程优化不同，下面用统一接口对比其收敛与精度。
...
try:
    ...
    xgb_clf = xgb.XGBClassifier(n_estimators=100, max_depth=4, learning_rate=0.1, verbosity=0)
    print(f"XGBoost CV: {cross_val_score(xgb_clf, X, y, cv=3).mean():.4f}")
except ImportError:
    print("XGBoost 未安装，跳过")

try:
    import lightgbm as lgb
    lgb_clf = lgb.LGBMClassifier(n_estimators=100, max_depth=4, learning_rate=0.1, verbose=-1)
    print(f"LightGBM CV: {cross_val_score(lgb_clf, X, y, cv=3).mean():.4f}")
except ImportError:
    print("LightGBM 未安装，跳过")

try:
    import catboost as cb
    cb_clf = cb.CatBoostClassifier(n_estimators=100, max_depth=4, learning_rate=0.1, verbose=False)
    print(f"CatBoost CV: {cross_val_score(cb_clf, X, y, cv=3).mean():.4f}")
except ImportError:
