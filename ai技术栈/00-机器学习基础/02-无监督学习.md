...
### 案例：用轮廓系数与 Calinski-Harabasz 自动选 K

不同指标对簇形状的偏好不同，下面对比两种内部指标选 K 的结果。
...
np.random.seed(42)
X = np.concatenate([
    np.random.randn(150, 2) + [3, 3],
    np.random.randn(150, 2) + [-3, 3],
    np.random.randn(150, 2) + [0, -3],
])

print(f"{'K':>2} | {'轮廓系数':>10} | {'Calinski-Harabasz':>18}")
for k in range(2, 7):
    km = KMeans(n_clusters=k, n_init=10, random_state=42).fit(X)
    sil = silhouette_score(X, km.labels_)
    ch = calinski_harabasz_score(X, km.labels_)
    print(f"{k:>2} | {sil:>10.4f} | {ch:>18.1f}")
```
...
### 案例：PCA 白化与 Kernel PCA 非线性降维

PCA 只能捕捉线性结构；Kernel PCA 通过核技巧映射到高维再降维，可分离环形数据。
...
X_pca = pca.fit_transform(X)
print(f"PCA 解释方差比: {pca.explained_variance_ratio_.round(3)}")

kpca = KernelPCA(n_components=2, kernel="rbf", gamma=10)
X_kpca = kpca.fit_transform(X)
print(f"KernelPCA 输出形状: {X_kpca.shape}, 前两维方差: {X_kpca.var(0).round(3)}")

# 白化: 各主成分单位方差
pca_white = PCA(n_components=2, whiten=True)
X_white = pca_white.fit_transform(X)
print(f"白化后每维标准差(应≈1): {X_white.std(0).round(3)}")
```
