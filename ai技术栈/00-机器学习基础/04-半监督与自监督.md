...
### 案例：伪标签自训练端到端对比（不同标注比例）

标注数据越少，自训练增益越明显。下面扫描 1%/5%/10% 标注率下的性能提升。
...
for label_ratio in [0.01, 0.05, 0.10]:
    rng = np.random.RandomState(42)
    mask = rng.rand(len(y)) < label_ratio
    y_semi = y.copy()
    y_semi[~mask] = -1

    base = RandomForestClassifier(n_estimators=100, random_state=42)
    st = SelfTrainingClassifier(base, threshold=0.7).fit(X, y_semi)

    base_only = RandomForestClassifier(n_estimators=100, random_state=42)
    base_only.fit(X[mask], y[mask])

    gain = st.score(X, y) - base_only.score(X, y)
    print(f"标注率={label_ratio:.0%}: 仅标注={base_only.score(X, y):.4f} "
          f"自训练={st.score(X, y):.4f} 提升={gain:+.4f}")
```
