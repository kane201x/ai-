...
### 案例：Target Encoding 的目标泄露对比

直接对所有数据求目标均值会泄露标签。下面对比"全量编码"与"K-Fold 交叉编码"的验证集表现。
...
np.random.seed(42)
X_raw = np.random.choice(["A", "B", "C", "D"], size=1000)
X_num = np.random.randn(1000, 3)
y = (X_num[:, 0] + (X_raw == "A").astype(float) > 0).astype(int)

def target_encode(X_cat, y, kf):
    out = np.zeros_like(X_cat, dtype=float)
    global_mean = y.mean()
    for tr, va in kf.split(X_cat):
        means = {c: y[tr][X_cat[tr] == c].mean() if (X_cat[tr] == c).any() else global_mean
                 for c in np.unique(X_cat)}
        out[va] = np.array([means[c] for c in X_cat[va]])
    return out

# 错误: 全量均值(泄露)
leak_enc = np.array([y[X_raw == c].mean() for c in X_raw])
# 正确: K-Fold 编码
safe_enc = target_encode(X_raw, y, KFold(5, shuffle=True, random_state=42))

for name, enc in [("泄露编码", leak_enc), ("K-Fold编码", safe_enc)]:
    Xf = np.column_stack([X_num, enc.reshape(-1, 1)])
    tr, te = range(800), range(800, 1000)
    clf = LogisticRegression().fit(Xf[tr], y[tr])
    print(f"{name} 验证准确率: {clf.score(Xf[te], y[te]):.4f}")
```
