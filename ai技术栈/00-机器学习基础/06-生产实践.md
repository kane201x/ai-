...
生产环境常用 shadow mode 验证新模型后再切换流量。下面模拟两版模型的预测一致性检查。
...
# 旧模型(v1)与新模型(v2)
v1 = RandomForestClassifier(n_estimators=50, random_state=1).fit(X, y)
v2 = RandomForestClassifier(n_estimators=200, random_state=2).fit(X, y)

# 影子模式: 同时预测, 比较分布差异
p1, p2 = v1.predict_proba(X)[:, 1], v2.predict_proba(X)[:, 1]
print(f"v1 准确率: {v1.score(X, y):.4f}, v2 准确率: {v2.score(X, y):.4f}")
print(f"两版预测概率 MAE: {mean_absolute_error(p1, p2):.4f}")
print(f"预测分歧样本占比: {np.mean(np.sign(p1 - 0.5) != np.sign(p2 - 0.5)):.2%}")

# 上线决策: 仅当 v2 更优且分歧可控时切换
if v2.score(X, y) > v1.score(X, y) and mean_absolute_error(p1, p2) < 0.1:
    print("✅ 满足切换条件: 升级到 v2")
else:
    print("⏸️ 维持 v1, 继续观察")
```
