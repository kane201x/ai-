...
## 7. 案例：工业级训练循环模板（AMP + 梯度累积 + 调度）

整合混合精度、梯度累积、余弦调度与梯度裁剪的完整可复用训练函数。

```mermaid
graph TD
    A["取 batch x,y"] --> B["BF16 autocast 前向"]
    B --> C["loss / accumulate"]
    C --> D["backward 累积梯度"]
    D --> E{"满 accum 步?"}
    E -->|否| A
...
def train_epoch(model, loader, opt, sched, scaler, accum: int = 4, device="cuda"):
    model.train()
    total_loss = 0.0
    opt.zero_grad()
...
## 8. 案例：优化器横向对比（同任务不同优化器）

在同一玩具任务上对比 SGD / AdamW / Lion 的收敛速度，理解「为何 Transformer 首选 AdamW」。

| 优化器 | 显存占用 | 收敛速度 | 所需 warmup | 适用 |
|--------|---------|---------|------------|------|
| SGD+Momentum | 低 | 慢(需调 LR) | 短 | CV/小模型 |
| AdamW | 中(存 m,v) | 快 | 需要 | Transformer 默认 |
| Lion | 低(仅 m) | 快 | 需要 | 大模型/省显存 |
| Muon | 中 | 快(2D 权重) | 需要 | LLM 预训练 |

```python
import torch

def toy_train(optim_cls, lr=1e-3, steps=200):
    torch.manual_seed(0)
    w = torch.randn(10, requires_grad=True)
    target = torch.randn(10)
    opt = optim_cls([w], lr=lr) if optim_cls is not Lion else Lion([w], lr=lr)
    for _ in range(steps):
        loss = ((w - target) ** 2).sum()
        opt.zero_grad(); loss.backward(); opt.step()
    return float(((w - target) ** 2).sum())

# 注意：Lion 需上文定义的类；此处仅演示调用范式
print("SGD 末态误差(示意):", round(toy_train(torch.optim.SGD, lr=0.1), 4))
```
