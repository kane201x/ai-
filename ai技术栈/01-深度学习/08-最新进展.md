...
```mermaid
graph TD
    CENTER["2025-2026 深度学习趋势"] --> A["推理时扩展\nTest-Time Compute"]
    CENTER --> B["MoE 规模化\n万亿参数稀疏激活"]
    CENTER --> C["Agent 自主化\nMCP / A2A 协议"]
    CENTER --> D["多模态统一\n端到端视觉语言"]
    CENTER --> E["效率优先\n量化/蒸馏/剪枝"]
...
## 7. 案例：MoE 负载均衡与无辅助损失路由

演示专家利用率监控与「辅助损失无关」的偏置更新策略，缓解专家坍缩。

```mermaid
graph LR
    A["Token 表示 x"] --> B["Router Gate\n→专家分数"]
    B --> C["Top-K 选择\n如 K=2"]
    C --> D["专家计算\n加权求和"]
    D --> E["更新专家偏置\n均衡负载"]
    E --> F["输出"]
...
class BalancedMoERouter(torch.nn.Module):
    def __init__(self, d_model: int, n_experts: int = 8, top_k: int = 2):
        super().__init__()
        self.gate = torch.nn.Linear(d_model, n_experts)
        self.bias = torch.nn.Parameter(torch.zeros(n_experts))   # 负载均衡偏置
        self.top_k = top_k

    def forward(self, x: torch.Tensor):
        logits = self.gate(x) + self.bias                        # 加偏置
        topk_val, topk_idx = logits.topk(self.top_k, -1)
        w = F.softmax(topk_val, -1)
        # 统计每个专家被选中的频率（用于更新 bias）
        counts = torch.zeros(self.bias.size(0), device=x.device)
        counts.scatter_add_(0, topk_idx.view(-1), torch.ones(topk_idx.numel(), device=x.device))
        return topk_idx, w, counts                               # counts: [n_experts]

x = torch.randn(64, 256)
router = BalancedMoERouter(256, n_experts=8, top_k=2)
idx, w, counts = router(x)
print("各专家被选中 token 数:", counts.tolist())
print("负载最重/最轻专家差:", int(counts.max() - counts.min()))
```

## 8. 案例：推理时计算预算控制（Test-Time Compute）

用「预算强制」控制思考步数：预算越大，生成与验证的 Chain-of-Thought 越长。
...
def reasoning_with_budget(model, question: str, budget: int = 5) -> str:
    """伪代码：按预算重复「生成→验证」循环。budget 越大思考越久。"""
    ctx = question
    for step in range(budget):
        draft = model.generate(ctx + f"\n思考第{step+1}步:")
        verdict = model.verify(draft)              # 验证器返回 bool
        if verdict:
            return draft                            # 通过即返回
        ctx = ctx + "\n" + draft + "\n(未通过, 重试)"
    return ctx                                      # 预算耗尽返回最优尝试

print("预算=3 调用示例:", "reasoning_with_budget(model, '2+3*4=?', budget=3)")
```
