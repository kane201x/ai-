...
正文「其他 PEFT 方法」提到 Adapter 在 Transformer 层插入瓶颈 MLP，下面给出可运行实现：
...
class Adapter(nn.Module):
    ...
    def forward(self, x):
        residual = x
        h = self.drop(self.act(self.down(x)))
        return residual + self.scale * self.up(h)

class TransformerBlockWithAdapter(nn.Module):
    def __init__(self, d_model, n_head, bottleneck=64):
        super().__init__()
        self.attn = nn.MultiheadAttention(d_model, n_head, batch_first=True)
        self.ffn = nn.Sequential(nn.Linear(d_model, d_model*4), nn.GELU(), nn.Linear(d_model*4, d_model))
        self.norm = nn.LayerNorm(d_model)
        # 注意力子层后、FFN 子层后各插一个 Adapter
        self.adapter_attn = Adapter(d_model, bottleneck)
        self.adapter_ffn = Adapter(d_model, bottleneck)

    def forward(self, x):
        h = self.norm(x)
        x = x + self.attn(h, h, h)[0]
        x = self.adapter_attn(x)            # 注意力后 Adapter
        h2 = self.norm(x)
        x = x + self.ffn(h2)
        x = self.adapter_ffn(x)             # FFN 后 Adapter
        return x

model = TransformerBlockWithAdapter(256, 8, bottleneck=32)
x = torch.randn(2, 16, 256)
print("Adapter 输出形状:", tuple(model(x).shape))
```

### 案例：LoRA / QLoRA / 全参 微调显存与可训练参数估算

下表基于 7B 模型、batch=4、seq=2048、优化器状态按 AdamW（12 字节/参数）估算，帮助选型：

| 方法 | 可训练参数 | 权重显存 | 优化器状态 | 总计估算显存 | 能否单卡 24G |
|------|-----------|---------|-----------|------------|------------|
| 全参 FT (FP16) | 7.0B (100%) | 14GB | 84GB | ~100GB | 否 |
| LoRA (r=16) | ~21M (0.3%) | 14GB | 0.25GB | ~16GB | 是 |
| QLoRA (NF4) | ~21M (0.3%) | 3.5GB | 0.13GB | ~5GB | 是（余量充足） |
| DoRA (r=16) | ~35M (0.5%) | 14GB | 0.4GB | ~16GB | 是 |

```python
def estimate_memory(params_b, method, dtype_bytes=2):
    # 返回估算显存（GB）
    weights = params_b * dtype_bytes
    if method == "full":
        optimizer = params_b * 12  # AdamW: fp32 副本 + m + v
        trainable = params_b
    elif method in ("lora", "qlora", "dora"):
        trainable = params_b * 0.003 if method != "dora" else params_b * 0.005
        optimizer = trainable * 12
        weights = weights * 0.25 if method == "qlora" else weights  # NF4 量化权重
    else:
        optimizer = 0
    return weights + optimizer  # 忽略激活/碎片

for m in ["full", "lora", "qlora", "dora"]:
    print(f"{m:6s} 7B 估算显存: {estimate_memory(7.0, m):.1f} GB")
```

### 案例：多阶段后训练流水线（2026 新范式）示意图

正文「多阶段后训练流水线（2026 新范式）」描述了先培养领域专家、再 On-policy Distill 整合。下图补全流程：

```mermaid
flowchart TB
    RAW["基座模型"] --> SFT1["SFT 数学专家"]
    RAW --> SFT2["SFT 代码专家"]
    RAW --> SFT3["SFT 医疗专家"]

    SFT1 --> GRPO1["GRPO 数学对齐"]
    SFT2 --> GRPO2["GRPO 代码对齐"]
    SFT3 --> GRPO3["GRPO 医疗对齐"]

    GRPO1 --> TEACH["10+ 领域教师模型"]
    GRPO2 --> TEACH
    GRPO3 --> TEACH

    TEACH --> DISTILL["On-policy Distillation\n多教师蒸馏"]
    DISTILL --> UNIFIED["统一模型"]
    UNIFIED --> EVAL["能力均衡评估"]
