...
### 案例：MHA / GQA / MQA 的 KV Cache 显存对比

正文「KV Cache 压缩」一节给出各策略压缩比，下面按 70B、seq=4K、d=128、层数 80 估算每请求的 KV 显存（FP16）：

```python
def kv_cache_gb(n_layers, seq_len, d_head, n_kv_heads, bytes_per=2):
    # KV 两端：2 × layers × seq × n_kv_heads × d_head × 字节
    elements = 2 * n_layers * seq_len * n_kv_heads * d_head
    return elements * bytes_per / (1024 ** 3)

n_layers, seq, d = 80, 4096, 128
print(f"MHA  (64 KV 头): {kv_cache_gb(n_layers, seq, d, 64):.2f} GB/请求")
print(f"GQA  (8 KV 头):  {kv_cache_gb(n_layers, seq, d, 8):.2f} GB/请求")
print(f"MQA  (1 KV 头):  {kv_cache_gb(n_layers, seq, d, 1):.2f} GB/请求")
print(f"MLA  (压缩到 r=64  latent): {kv_cache_gb(n_layers, seq, 64, 1):.2f} GB/请求  (约 MHA 的 1/10)")
```

| 注意力类型 | KV 头数 | 单请求 KV 显存 (seq=4K) | 相对 MHA |
|-----------|--------|----------------------|---------|
| MHA | 64 | 5.00 GB | 1× |
| GQA | 8 | 0.62 GB | 0.125× |
| MQA | 1 | 0.08 GB | 0.016× |
| MLA | latent r=64 | ~0.50 GB | ~0.1× |

### 案例：投机解码接受率模拟

对应正文「推测解码」一节，下面用真实分布近似验证「拒绝采样接受率 70-90%」：
...
gamma = 5
n_trials = 2000
# 假设草稿模型与目标模型分布接近（KL 小）
target_probs = torch.distributions.Categorical(probs=torch.rand(50000)).sample((1,))
accepted_total = 0
tokens_total = 0

for _ in range(n_trials):
    # 草稿采样 gamma 个 token（目标同分布 → 高接受率）
    draft = torch.randint(0, 50000, (gamma,))
    # 接受概率 = min(1, p_target / p_draft)，同分布下接近 1
    p_target = torch.ones(gamma) * 0.5
    p_draft = torch.ones(gamma) * 0.5
    accept = (torch.rand(gamma) < (p_target / p_draft).clamp(max=1.0))
    n_accept = accept.sum().item()
    accepted_total += n_accept
    tokens_total += gamma
    # 每个序列额外补 1 个 bonus token
    tokens_total += 1

print(f"平均接受率 ≈ {accepted_total / (n_trials * gamma):.1%}（越接近 1，加速越明显）")
```

### 案例：Prefill-Decode 分离部署示意图

正文「推理服务架构」提到 Prefill/Decode 分离，下图补全请求在两个阶段的不同资源占用：

```mermaid
flowchart LR
    subgraph PREFILL["Prefill 阶段 (计算密集)"]
        P1["整段 Prompt 并行计算"] --> P2["生成首 Token + 完整 KV Cache"]
        P2 --> P3["GPU 算力打满\n显存占用高"]
    end

    subgraph DECODE["Decode 阶段 (带宽密集)"]
        D1["逐 Token 自回归"] --> D2["每次仅算 1 个新 Token"]
        D2 --> D3["GPU 算力闲置\n受 KV 带宽限制"]
    end

    REQ["用户请求"] --> PREFILL
    PREFILL -->|"KV Cache 移交"| DECODE
    DECODE -->|"流式输出"| OUT["客户端"]

    SEP["两套 GPU 池分离调度"] --> PREFILL
    SEP --> DECODE
```
