...
class PatchEmbed(nn.Module):
    def __init__(self, img_size: int = 224, patch: int = 16, in_c: int = 3, embed: int = 768):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return x.flatten(2).transpose(1, 2)    # [B, num_patches, embed]

class ViTBlock(nn.Module):
    def __init__(self, embed: int = 768, heads: int = 12):
        super().__init__()
        ...
        self.mlp = nn.Sequential(
            nn.Linear(embed, embed * 4), nn.GELU(), nn.Linear(embed * 4, embed))
...
class MiniViT(nn.Module):
    def __init__(self, num_classes: int = 10, depth: int = 6):
        super().__init__()
        self.patch_embed = PatchEmbed()
        self.cls_token = nn.Parameter(torch.zeros(1, 1, 768))
        self.pos_embed = nn.Parameter(torch.zeros(1, 197, 768))
        self.blocks = nn.Sequential(*[ViTBlock() for _ in range(depth)])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.blocks(x)
        return self.head(x[:, 0])    # 取 CLS token 输出分类
```
...
def distillation_loss(student_logits: torch.Tensor, teacher_logits: torch.Tensor,
                       labels: torch.Tensor, alpha: float = 0.5, temp: float = 4.0) -> torch.Tensor:
    """软标签 + 硬标签联合监督，让小 ViT 学习大模型分布。"""
