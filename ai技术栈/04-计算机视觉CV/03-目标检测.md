...
### 案例：YOLOv8 推理 + 后处理（置信度过滤 + NMS）

```python
import torch
from typing import List, Dict

def postprocess_yolo(raw: torch.Tensor, conf_thresh: float = 0.25,
                     iou_thresh: float = 0.5) -> List[Dict]:
    """原始输出 [B, N, 4+nc] → 过滤后检测结果列表。"""
    results: List[Dict] = []
    for batch in raw:
        boxes, scores, labels = [], [], []
        for det in batch:
            box = det[:4]
            cls_scores = det[4:]
            best = torch.argmax(cls_scores)
            if cls_scores[best] < conf_thresh:
                continue
            boxes.append(box)
            scores.append(cls_scores[best])
            labels.append(best.item())
        if not boxes:
            results.append({"boxes": [], "scores": [], "labels": []})
            continue
        boxes_t = torch.stack(boxes)
        scores_t = torch.stack(scores)
        keep = non_max_suppression(boxes_t, scores_t, iou_thresh)
        results.append({
            "boxes": boxes_t[keep].tolist(),
            "scores": scores_t[keep].tolist(),
            "labels": [labels[i] for i in keep],
        })
    return results

# 使用 Detect 输出的伪代码调用
preds = model(torch.randn(1, 3, 640, 640))
detections = postprocess_yolo(preds)
```
...
def detr_postprocess(outputs: dict, threshold: float = 0.7) -> list:
    """DETR 输出已无 NMS：100 个 object queries 直接给出框与类别。"""
    logits = outputs["pred_logits"].softmax(-1)[0, :, :-1]  # 去掉 "no object" 类
    boxes = outputs["pred_boxes"][0]
    keep = logits.max(-1).values > threshold
    return [{"score": logits[i].max().item(),
             "label": logits[i].argmax().item(),
             "box": boxes[i].tolist()} for i in torch.where(keep)[0]]
```
...
### 检测范式对比（Anchor / Anchor-free / Query）

| 范式 | 代表模型 | 是否需要 Anchor/NMS | 预测单位 | 训练难点 |
|------|---------|-------------------|---------|---------|
| Anchor-based | Faster R-CNN, RetinaNet | 需 Anchor，需 NMS | 预设锚框偏移 | 匹配策略敏感 |
| Anchor-free | YOLOX, FCOS, CenterNet | 无 Anchor，需 NMS | 关键点/距离 | 正负样本定义 |
| Query-based | DETR, DINO, RT-DETR | 无 Anchor，无 NMS | Object Queries | 二分图匹配/收敛慢 |

```mermaid
flowchart LR
    A[输入图像] --> B[Backbone 提取特征]
    B --> C{检测范式}
    C -->|Anchor-based| D[生成 Anchor + 回归/分类]
    C -->|Anchor-free| E[关键点/中心度预测]
    C -->|Query-based| F[Transformer 解码 Queries]
    D --> G[NMS 抑制冗余框]
    E --> G
    F --> H[直接输出最终框]
    G --> I[检测结果]
