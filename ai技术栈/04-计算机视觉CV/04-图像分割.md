...
def sam_point_segment(image: torch.Tensor, points: List[Tuple[int, int]],
                      labels: List[int], sam_model) -> torch.Tensor:
    """给定图像与若干正/负点提示，返回二值掩码（小尺寸需上采样回原图）。"""
    with torch.no_grad():
        img_emb = sam_model.image_encoder(image)
        # points: [N,2]，labels: 1 正点 / 0 负点
        sparse = sam_model.prompt_encoder(
            points=(torch.tensor(points, dtype=torch.float32),
                    torch.tensor(labels)))
        masks, _, _ = sam_model.mask_decoder(img_emb, sparse, None, None)
    return masks[0]  # 返回最佳掩码 [1, H, W]

# 示例：点击前景点(100,120)与背景点(100,160)
binary_mask = sam_point_segment(
    torch.rand(1, 3, 1024, 1024),
    [(100, 120), (100, 160)], [1, 0], sam_model)
```

### 案例：SegFormer 风格 MLP 解码器
...
class SegFormerMLPDecoder(nn.Module):
    """聚合多层级 Transformer 特征，经 MLP 上采样输出分割图。"""
    def __init__(self, dims=(256, 512, 1024, 2048), embed=256, num_classes=21):
        super().__init__()
        self.projects = nn.ModuleList([
            nn.Conv2d(d, embed, 1) for d in dims])
        self.merge = nn.Conv2d(len(dims) * embed, embed, 1)
        self.out = nn.Sequential(
            nn.Conv2d(embed, embed, 3, 1, 1), nn.ReLU(),
            nn.Conv2d(embed, num_classes, 1))

    def forward(self, feats: list) -> torch.Tensor:
        ups = [nn.functional.interpolate(
            self.projects[i](f), size=feats[0].shape[2:], mode="bilinear",
            align_corners=False) for i, f in enumerate(feats)]
        x = self.merge(torch.cat(ups, dim=1))
        return self.out(x)
```
