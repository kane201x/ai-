...
### 案例：DDIM 加速采样（50 步→10 步）

```python
import torch

@torch.no_grad()
def ddim_sample(model: torch.nn.Module, diffusion, shape: tuple,
                steps: int = 10, eta: float = 0.0, device: str = "cuda") -> torch.Tensor:
    """DDIM 确定性采样：在子序列时间步上跳步去噪，速度提升数倍。"""
    ts = torch.linspace(0, diffusion.n_steps - 1, steps).long().to(device)
    x = torch.randn(shape, device=device)
    for i in range(steps - 1, -1, -1):
        t = ts[i]
        t_next = ts[i - 1] if i > 0 else torch.tensor(0, device=device)
        pred_noise = model(x, t.unsqueeze(0).expand(shape[0]))
        alpha_bar = diffusion.alpha_bars[t].to(device)
        alpha_bar_next = diffusion.alpha_bars[t_next].to(device)
        x0_pred = (x - (1 - alpha_bar).sqrt() * pred_noise) / alpha_bar.sqrt()
        dir = (1 - alpha_bar_next).sqrt() * pred_noise
        x = alpha_bar_next.sqrt() * x0_pred + dir
    return x.clip(-1, 1)
```

### 案例：潜在扩散文生图（Stable Diffusion 式）流程
