...
### 案例：CLIP 零样本分类（多 prompt 集成）

```python
import torch

def clip_zero_shot(image: torch.Tensor, class_names: list,
                   model, templates=None) -> list:
    """用多个文本模板描述每个类别，取最高相似度作为预测。"""
    if templates is None:
        templates = ["一张{}的照片", "这是{}", "{}的图像"]
    text_prompts = [t.format(name) for name in class_names for t in templates]
    tokens, mask = SimpleTokenizer().encode(text_prompts)
    with torch.no_grad():
        img_feat = model.image_encoder(image.unsqueeze(0))
        txt_feat = model.text_encoder(tokens.cuda(), mask.cuda())
        logits = model.logit_scale.exp() * img_feat @ txt_feat.t()
    # 每个类别取其模板分数的平均
    per_class = logits.view(1, len(class_names), len(templates)).mean(-1)
    return torch.softmax(per_class, dim=1).squeeze().tolist()
```

### 案例：单图多轮视觉问答（VLM 推理流程）

```mermaid
flowchart TD
    A[输入图像] --> B[视觉编码器 ViT]
    B --> C[视觉 Token 序列]
    D[用户问题文本] --> E[Tokenizer]
    E --> F[文本 Token]
    C --> G[投影/适配层]
    G --> H[LLM 自回归解码]
    F --> H
