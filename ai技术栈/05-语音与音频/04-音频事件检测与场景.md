...
### 案例：用 BEATs 做零样本音频事件检测

BEATs 提供通用音频表征，配合 CLAP 式文本提示可实现开放词汇事件检测。

```python
import torch
import torch.nn.functional as F

def open_vocabulary_sed(audio_emb, text_emb, event_names, temperature=0.07):
    """用音频嵌入与文本提示的相似度判断事件是否出现"""
    a = F.normalize(audio_emb, dim=-1)
    t = F.normalize(text_emb, dim=-1)
    logits = torch.matmul(a, t.T) / temperature  # [1, N_events]
    probs = F.sigmoid(logits).squeeze(0)
    return {name: float(p) for name, p in zip(event_names, probs)}

audio_emb = torch.randn(1, 768)
text_emb = torch.randn(5, 768)
events = ["狗叫", "警笛", "雨声", "笑声", "钢琴"]
scores = open_vocabulary_sed(audio_emb, text_emb, events)
...
结合 CRNN 与滑窗推理，对长音频做连续事件检测并统计各类事件频次。

```python
import torch
import numpy as np

def sliding_window_detect(model, log_mel, win_len=200, hop=100, threshold=0.5):
    """对长音频做滑窗事件检测，返回事件计数"""
    counts = {}
    n_frames = log_mel.shape[-1]
    for start in range(0, n_frames - win_len + 1, hop):
        seg = log_mel[:, :, start:start + win_len]
        with torch.no_grad():
            probs = model(seg).squeeze(0)
...
model = CRNN(n_classes=10)
long_mel = torch.randn(1, 64, 2000)
result = sliding_window_detect(model, long_mel, win_len=200, hop=100)
print(f"检测到的事件类别计数: {result}")
```

### 案例：音频场景分类弱监督训练

仅在整段音频有场景标签时，用注意力池化聚合帧级预测即可训练 ASC 模型。
...
class WeaklySupervisedASC(nn.Module):
    """弱监督场景分类：帧级预测 + 注意力池化 -> 整段标签"""
    def __init__(self, n_classes=10, d=128):
        super().__init__()
        self.frame_net = nn.Sequential(
            nn.Conv1d(64, d, 3, padding=1), nn.ReLU(),
    ...
    def forward(self, mel):
        x = self.frame_net(mel)
        B, D, T = x.shape
        a = F.softmax(self.attn(x.transpose(1, 2)), dim=1)
        seg = (x.transpose(1, 2) * a).sum(dim=1)
        return self.cls(seg)

model = WeaklySupervisedASC(n_classes=10)
mel = torch.randn(4, 64, 200)
logits = model(mel)
print(f"场景 logits: {logits.shape}")
```
...
| 任务 | 输入 | 输出 | 代表模型 |
|------|------|------|---------|
| SED | 长音频 | 事件+起止 | CRNN / BEATs |
| ASC | 整段音频 | 单个场景 | PANNs / AST |
| 开放词汇 | 音频+文本 | 任意事件 | CLAP / 文本提示 |
| 边缘检测 | 实时流 | 事件告警 | TinyML CRNN |
