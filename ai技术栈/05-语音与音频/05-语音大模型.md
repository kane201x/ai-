...
### 案例：端到端语音对话（GPT-4o 风格）

取消 ASR+TTS 中间文本，语音编码器与语音解码器直接对接 LLM，实现低延迟对话。

```python
import torch
import torch.nn as nn

class EndToEndSpeechLLM(nn.Module):
    """简化版端到端语音对话模型：语音输入 -> LLM -> 语音输出"""
    def __init__(self, audio_enc, speech_dec, llm):
        super().__init__()
        self.audio_enc = audio_enc
        self.speech_dec = speech_dec
        self.llm = llm

    def forward(self, speech_input):
        semantic = self.audio_enc(speech_input)
        resp_tokens = self.llm.generate(semantic)
        resp_speech = self.speech_dec(resp_tokens)
        return resp_speech

# 仅作结构示意，实际组件需替换为预训练权重
audio_enc = nn.Linear(80, 512)
llm = type("LLM", (object,), {"generate": lambda self, x: torch.randint(0, 100, (1, 20))})
speech_dec = nn.Linear(512, 80)
model = EndToEndSpeechLLM(audio_enc, speech_dec, llm)
print("端到端语音对话模型已构建")
```
...
将语音文档切分为片段并编码，检索与查询最相关的片段后喂给 LLM 回答。
...
class VoiceRAG:
    """语音文档检索增强：查询语音检索相关语音片段"""
    def __init__(self, encoder):
        self.encoder = encoder
        self.docs = []
        self.doc_embs = None

    def index(self, segments):
        self.docs = segments
        self.doc_embs = F.normalize(torch.stack([self.encoder(s) for s in segments]), dim=-1)

    def retrieve(self, query, top_k=2):
        q = F.normalize(self.encoder(query), dim=-1)
        sims = torch.matmul(q, self.doc_embs.T).squeeze(0)
        idx = torch.argsort(-sims)[:top_k]
        return [self.docs[i] for i in idx]

encoder = lambda x: torch.randn(256)
rag = VoiceRAG(encoder)
rag.index([torch.randn(80, 100) for _ in range(5)])
hits = rag.retrieve(torch.randn(80, 100), top_k=2)
print(f"检索到 {len(hits)} 个相关语音片段")
```

### 案例：多模态语音理解（语音+视觉）

语音与图像特征拼接后统一送入 LLM，实现看图听音的多模态问答。

```python
import torch
import torch.nn as nn

class MultimodalSpeechLLM(nn.Module):
    def __init__(self, audio_dim=512, vision_dim=768, llm_dim=4096):
        super().__init__()
    ...
    def forward(self, audio_feat, vision_feat):
        ...
        return fused  # 拼接后作为 LLM 的 inputs_embeds

model = MultimodalSpeechLLM()
audio = torch.randn(2, 1500, 512)
vision = torch.randn(2, 257, 768)
fused = model(audio, vision)
print(f"多模态融合嵌入: {fused.shape}")
```
