...
下面用一个 4×4 网格世界（类似 FrozenLake 但确定转移）演示策略迭代的完整流程，并可视化价值函数收敛。

```python
import numpy as np

# 4x4 网格世界，终点在右下角(3,3)，到达奖励 0，其余每步 -1
GRID = 4
TERMINAL = (3, 3)
ACTIONS = {0: (-1, 0), 1: (1, 0), 2: (0, -1), 3: (0, 1)}  # 上 下 左 右

def get_transitions(s, a):
    # 返回 [(p, r, s_next), ...]，这里为确定性转移
    if s == TERMINAL:
        return [(1.0, 0.0, s)]
    r, c = s
    dr, dc = ACTIONS[a]
    ...
    s_next = (nr, nc)
    reward = 0.0 if s_next == TERMINAL else -1.0
    return [(1.0, reward, s_next)]

def idx(s):
    return s[0] * GRID + s[1]

# 复用文件 4.2 的 ValueIteration 思路，这里直接展开一次策略迭代
n_states = GRID * GRID
n_actions = 4
...
for it in range(50):
    ...
    while True:
        ...
        for s in range(n_states):
            v = values[s]
            r, c = divmod(s, GRID)
            transitions = get_transitions((r, c), policy[s])
            values[s] = sum(p * (rew + gamma * values[idx(sn)]) for p, rew, sn in transitions)
    ...
    for s in range(n_states):
        r, c = divmod(s, GRID)
        q = [sum(p * (rew + gamma * values[idx(sn)]) for p, rew, sn in get_transitions((r, c), a))
        ...
        if best != policy[s]:
            ...
            stable = False
    if stable:
        print(f"策略迭代在第 {it + 1} 轮收敛")
        break

# 打印价值函数热力（每步 -1 的代价下，越靠近终点价值越高）
print("价值函数网格:")
for r in range(GRID):
    print(" ".join(f"{values[idx((r, c))]:7.2f}" for c in range(GRID)))
```

### 案例：ε-greedy 与 UCB 探索效率对比

在静态多臂老虎机上比较不同探索策略的累计后悔（regret），直观理解探索-利用权衡。
...
np.random.seed(42)
n_arms = 5
true_means = np.random.uniform(0.0, 1.0, n_arms)  # 真实奖励均值
best_arm = int(np.argmax(true_means))
steps = 2000

def run_epsilon(steps, eps=0.1):
    counts = np.zeros(n_arms); values = np.zeros(n_arms)
    regret = 0
    for t in range(steps):
        ...
        r = np.random.normal(true_means[a], 0.1)
        counts[a] += 1
        values[a] += (r - values[a]) / counts[a]
        regret += true_means[best_arm] - true_means[a]
    return regret

def run_ucb(steps):
    counts = np.zeros(n_arms); values = np.zeros(n_arms)
    total = 0; regret = 0
    for a in range(n_arms):  # 每臂先试一次
        r = np.random.normal(true_means[a], 0.1)
        counts[a] += 1; total += 1
        values[a] = r
    for t in range(n_arms, steps):
        ucb = values + np.sqrt(2 * np.log(total) / counts)
        a = int(np.argmax(ucb))
        r = np.random.normal(true_means[a], 0.1)
        counts[a] += 1; total += 1
        values[a] += (r - values[a]) / counts[a]
...
print(f"真实最优臂均值 = {true_means[best_arm]:.3f}")
print(f"ε-greedy 累计后悔 = {run_epsilon(steps):.1f}")
print(f"UCB 累计后悔      = {run_ucb(steps):.1f}")
```
