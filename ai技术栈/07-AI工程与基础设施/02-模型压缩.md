...
### 案例：LLaMA-7B INT4 AWQ 量化并对比精度/显存

下面演示使用 AutoAWQ 对 LLaMA-2-7B 做 4-bit 量化，并量化前后对比显存占用与困惑度（PPL）。

```python
# LLaMA-7B AWQ 量化 + 效果对比
...
model_fp16 = AutoAWQForCausalLM.from_pretrained(model_name, device_map="cuda")
mem_fp16 = torch.cuda.max_memory_allocated(0) / 1024**3

torch.cuda.reset_peak_memory_stats(0)
model_int4 = AutoAWQForCausalLM.from_pretrained("./llama2-7b-awq-int4", device_map="cuda")
mem_int4 = torch.cuda.max_memory_allocated(0) / 1024**3

print(f"FP16 显存: {mem_fp16:.2f} GB")
print(f"INT4 显存: {mem_int4:.2f} GB")
print(f"显存压缩比: {mem_fp16 / mem_int4:.2f}x")
```

### 实现案例：QLoRA 4-bit 微调（NF4 量化训练）

```python
# QLoRA 4-bit 微调（bitsandbytes + PEFT）
import torch
...
bnb_config = BitsAndBytesConfig(
    load_in_4bit=True,
    bnb_4bit_quant_type="nf4",          # NF4 归一化浮点
...
| 方法 | 典型位宽 | 是否需要校准集 | 硬件要求 | 精度损失 | 适用场景 |
|------|---------|--------------|---------|---------|---------|
| PTQ (INT8) | 8-bit | 是 (少量) | 通用 INT8 | <0.5% | CNN、小模型 |
| GPTQ (INT4) | 4-bit | 是 (校准) | 通用 INT4 | 0.5-2% | LLM 发布部署 |
| AWQ (INT4) | 4-bit | 是 (校准) | 通用 INT4 | 0.5-1.5% | LLM 推理首选 |
| SmoothQuant | 8/4-bit | 是 (校准) | 激活需支持 | <1% | 激活难量化模型 |
| FP8 (E4M3) | 8-bit | 否 | H100/B200 | <0.3% | 新一代训练推理 |
| NF4 (QLoRA) | 4-bit | 否 | 通用 | 1-2% | 低资源微调 |
...
flowchart TD
    ...
    B -->|否 + 新硬件| D["FP8 E4M3"]
    B -->|是| E{"目标位宽?"}
    E -->|8-bit| F["PTQ INT8 / SmoothQuant"]
    E -->|4-bit| G{"侧重推理速度还是精度?"}
    G -->|速度优先| H["GPTQ INT4"]
    G -->|精度优先| I["AWQ INT4"]
    C --> J["低显存训练"]
    D --> J
