...
下面用 vLLM 的 benchmarking 工具对比开启/关闭连续批处理（`--enable-prefix-caching` + 默认 continuous batching）的吞吐差异。
...
# 用官方 benchmark 脚本压测（共享系统 prompt 场景）
python benchmarks/benchmark_serving.py \
    ...
    --request-rate 32 \
    --save-result \
    --result-filename vllm_cont_batching.json
```
...
with open("vllm_cont_batching.json") as f:
    r = json.load(f)

print(f"平均吞吐: {r['request_throughput']:.2f} req/s")
print(f"平均 TTFT: {r['mean_ttft_ms']:.1f} ms")
print(f"平均 ITL:  {r['mean_inter_token_latency_ms']:.1f} ms")
print(f"GPU 利用率: {r['gpu_utilization']*100:.1f}%")
```

### 实现案例：Triton 模型仓库 + 集成 vLLM 后端
...
#   llama3/
#     config.pbtxt
#     # 使用 vLLM 作为推理后端（Python 封装）

cat > model_repository/llama3/config.pbtxt << 'EOF'
name: "llama3"
platform: "python"
max_batch_size: 256
input [
...
| 服务器 | 核心能力 | 动态批处理 | 多模型 | 多框架 | 适用场景 |
|--------|---------|-----------|-------|-------|---------|
| vLLM | PagedAttention、高吞吐 LLM | ✅ 连续批 | ❌ (单一 LLM) | HF 模型 | 纯 LLM 服务 |
| NVIDIA Triton | 统一推理服务器 | ✅ 动态批 | ✅ | TF/PyTorch/ONNX/TensorRT | 多模型混合部署 |
| TGI | HF 官方 LLM 服务 | ✅ 连续批 | ❌ | HF 生态 | HF 模型托管 |
| SGLang | RadixAttention 结构化 | ✅ | ❌ | HF 模型 | 复杂结构化生成 |
| BentoML | 打包+服务一体化 | ⚠️ 基础 | ✅ | 多框架 | 快速交付 Python 服务 |

### Mermaid: 推理请求全链路

```mermaid
flowchart LR
    A["客户端"] --> B["API Gateway 鉴权/限流"]
    B --> C["负载均衡"]
    C --> D["推理节点 vLLM"]
    D --> E["连续批处理引擎"]
