...
### 案例：扩散策略（Diffusion Policy）生成动作序列

扩散策略用条件扩散模型在动作空间中去噪，天然支持多模态动作分布。下面是极简实现：
...
class DiffusionPolicy(nn.Module):
    def __init__(self, obs_dim=32, action_dim=7, horizon=16, hidden=256):
        super().__init__()
        self.horizon = horizon
        self.time_emb = nn.Sequential(nn.Linear(64, hidden), nn.SiLU(), nn.Linear(hidden, hidden))
        self.obs_emb = nn.Linear(obs_dim, hidden)
        self.model = nn.Sequential(
            nn.Linear(hidden * 2 + action_dim, hidden), nn.ReLU(),
            nn.Linear(hidden, hidden), nn.ReLU(),
            nn.Linear(hidden, action_dim))

    def forward(self, noisy_action, t, obs):
        te = self.time_emb(torch.sin(t[:, None] * torch.arange(1, 65).float().to(t.device) / 64))
        oe = self.obs_emb(obs)
        h = torch.cat([noisy_action, te.expand(noisy_action.shape[0], -1), oe], dim=-1)
        return self.model(h)

    def sample(self, obs, steps=20):
        a = torch.randn(obs.shape[0], self.horizon, 7)
        for s in reversed(range(steps)):
            t = torch.full((obs.shape[0],), s / steps)
            pred = self.forward(a.view(-1, 7), t, obs.repeat_interleave(self.horizon, 0))
            a = a + 0.1 * pred.view_as(a)
        return a[:, 0]  # 取第一步动作执行
...
| 平台 | 开发方 | 物理引擎 | 适用性 | 是否开源 | 渲染质量 |
|------|--------|---------|--------|---------|---------|
| MuJoCo | DeepMind | 自带 | 通用操作/运动 | ✓ | 中 |
| Isaac Gym/Sim | NVIDIA | PhysX | 大规模GPU并行 | ✓ | 高 |
| PyBullet | 社区 | 自带 | 轻量/教学 | ✓ | 中 |
| SAPIEN | 斯坦福 | 自带 | 可交互场景 | ✓ | 高 |
| Habitat | Meta | 自带 | 视觉导航 | ✓ | 高 |
| CoppeliaSim | 商业 | 多引擎 | 工业仿真 | 部分 | 高 |
