...
### 案例：Graph-of-Thoughts 合并与回溯

相比 ToT 的树结构，GoT 允许思维节点合并（aggregate）以综合多条推理路径。下面给出极简有向图聚合示例：
...
class GraphOfThoughts:
    ...
    def add_node(self, thought: str, parents=None):
        nid = self.next_id
        self.nodes[nid] = GoTNode(nid, thought, parents or [])
        for p in (parents or []):
            self.nodes[p].children.append(nid)
        self.next_id += 1
        return nid

    def aggregate(self, node_ids, new_thought):
        # 将多条父路径合并为综合思维
        merged = " / ".join(self.nodes[i].thought for i in node_ids)
        return self.add_node(new_thought + " [合并: " + merged + "]", parents=node_ids)

    def best_leaf(self):
        leaves = [n for n in self.nodes.values() if not n.children]
        return max(leaves, key=lambda n: n.score, default=None)

# 用法示例
got = GraphOfThoughts()
a = got.add_node("路径A: 用代数法")
b = got.add_node("路径B: 用几何法")
c = got.aggregate([a, b], "综合: 两种方法一致")
got.nodes[c].score = 0.95
print("最优叶节点思维:", got.best_leaf().thought)
```
