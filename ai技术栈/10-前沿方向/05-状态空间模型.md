...
### 案例：用选择性 SSM 做序列分类（训练一步）

```python
import torch
import torch.nn as nn

# 复用上文 SimplifiedMamba 结构
class MambaClassifier(nn.Module):
    def __init__(self, d_model=64, d_state=16, n_layers=2, num_classes=4):
        super().__init__()
        # 这里用简化 SSM 块（见上文 SelectiveSSM）
        from types import SimpleNamespace
        self.embed = nn.Embedding(1000, d_model)
        self.blocks = nn.ModuleList([
            SimpleNamespace(ssm=__import__("torch").nn.Identity())  # 占位，实际替换为 SelectiveSSM
        ])
...
def train_one_step(model, batch_ids, labels, lr=1e-3):
...
ids = torch.randint(0, 1000, (8, 32))
labels = torch.randint(0, 4, (8,))
print("单步训练 loss:", train_one_step(model, ids, labels))
```

> 注：生产实现请使用 `mamba-ssm` 官方的 `Mamba` 层与 `cuda` 内核以获得硬件感知扫描加速。

### 案例：S4 vs Mamba 训练/推理模式对比
...
graph LR
    ...
    subgraph Mamba 循环模式(推理)
        A2[新 token] --> B2[更新状态 h]
        B2 --> C2[输出 y]
        C2 --> A2
    end
```
