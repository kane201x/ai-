...
### 案例：推理预算自适应分配（Test-time Scaling 落地）

根据问题难度动态分配推理算力，是 2026 年部署标准做法。下面给出路由示例：
...
def route_question(question: str):
    hard_kw = ["证明", "推导", "优化", "多步", "定理"]
    q = question.lower()
    if any(k in q for k in hard_kw):
        return ReasonMode.MAX, {"samples": 16, "tokens": 16000}
    med_kw = ["比较", "分析", "计算", "为什么"]
    if any(k in q for k in med_kw):
        return ReasonMode.DEEP, {"samples": 8, "tokens": 4096}
    return ReasonMode.FAST, {"samples": 1, "tokens": 256}

# 用法示例
mode, budget = route_question("请证明费马小定理")
print("路由模式:", mode.value, "预算:", budget)
```

### 案例：MoE 激活率对比（2026 主流动向）

```mermaid
graph LR
    A[2026 MoE] --> B[DeepSeek V4 3.1%]
    A --> C[Llama 4 4.3%]
    A --> D[Qwen3 9.4%]
    B --> E[更低激活=更低推理成本]
    C --> E
